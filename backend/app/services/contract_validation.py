...
def validate_sample_against_prompt_version(
    sample: SampleRecord,
    ...
    for slot in prompt.image_slot_specs:
    ...
    for spec in prompt.variable_specs:
...
def validate_records_against_contract(
    ...
    for index, record in enumerate(records, start=1):
        errors = validate_sample_against_prompt_version(record, prompt_version)
        if errors:
            invalid_rows.append(
